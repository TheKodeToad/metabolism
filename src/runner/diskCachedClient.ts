...
export class DiskCachedClient implements HTTPClient {
	...
	async getCached(
		...
		return await this.cache.use(key, async (ref) => {
			...
			if (entry && hasBody(entry)) {
				if (
					strategy.mode === HTTPCacheMode.Eternal
					|| this.options.assumeUpToDate
				) {
				...
				if (strategy.mode === HTTPCacheMode.CompareLocalDigest) {
					if (await this.compareLocalDigest(entry, strategy)) {
						logger.debug(
							`'${key}' is up-to-date (matching digest)`,
						);
						return this.makeResponse(entry);
					} else {
			...
			this.logger.debug(
				`Cache entry '${key}' ${entry ? "created" : "updated"} with body and headers from response`,
			);
	...
	async headCached(url: string | URL, key: string): Promise<Metadata> {
		return await this.cache.use(key, async (ref) => {
			...
			this.logger.debug(
				`Cache entry '${key}' ${entry ? "created" : "updated"} with headers from response`,
			);
				...
				return await this.retry(url.toString(), async () => {
					...
					for (const zipEntry of entries) {
						...
						const index = files.findIndex(
							(file) => file.path === zipEntry.filename,
						);
						...
						const content = await zipEntry.getData!(
							new TextWriter(),
						);
						...
						logger.debug(
							`Cache entry '${file.key}' updated from ZIP entry '${zipEntry.filename}' from '${url.toString()}'`,
						);
	...
	private async compareLocalDigest(
		entry: CacheEntryWithBody,
		strategy: CompareLocalDigestStrategy,
	): Promise<boolean> {
		const digestResult =
	...
	private parseHeaders(headers: Headers): Omit<CacheEntry, "body"> {
		...
		const lastModified =
			lastModifiedRaw ? new Date(lastModifiedRaw) : undefined;

		if (lastModified && isNaN(lastModified.getTime())) {
			throw new Error(
				`Invalid Last-Modified timestamp: '${lastModifiedRaw!}'`,
			);
		}
